...
for j=[1:numel(OMarr)]
    plot(t(:,j),z_t(:,j),'color',col(j),'LineWidth',1.2)
end
...
title('Redshift vs Time','Fontsize',16);
xlabel('Time (Gyr)' ,'FontSize',16);
ylabel('Redshift(t)','FontSize',16);
% Legend
...
%text(13, 6, '\Omega_M', 'FontSize', 16);
OMs = num2str(OMarr');
legend(OMs,'Location','NorthEast');
legend('boxoff');
hold('off');
