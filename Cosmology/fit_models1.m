...
%NOTE NEED TO LOOK AT EACH DATA SET INDIVIDUALLY, EACH COMES FROM A
%DIFFERENT SOURCE
%
clear all
...
% added by sholto to test each data set individually
id_choose = 50
...
% CHANGE THESE VALUES, CHANGE STEP SIZE AND MAX VALUES added by sholto
OM_lo   = 0.00;
OM_hi   = 2.0;
OM_step = 0.07;
...
%       but it makes it obvious what is happening.)
        for k=[1:mscrn]
            ...
            chi2_test0 = sum( (mu_model_norm - mu).^2 ./ muerr.^2 );
            
            p_model = OM(i);
            p_prior = .27;
            sig_prior = 0.03;
            
%             chi2_test = chi2_test0 + ((p_model - p_prior)/sig_prior)^2;
...
%sholto comment: look at difference in chi2!

% Print out the values for the best fit parameters:
% sholto comment: reduced chi is important!
